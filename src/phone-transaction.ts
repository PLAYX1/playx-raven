...
export function wirePhoneTransaction(invoke:Invoke,t:(s:string)=>string) {
  ...
  function line(label:string,value:string|(()=>string)) {
    ...
    setCopyText(b, () => t(label)+': ');if(typeof value==='function')setCopyText(span,value);else {span.textContent=value;span.setAttribute('translate','no');}p.append(b,span);preview.append(p);
  }
  check.addEventListener('click',async()=>{
    if(sending)return;
    invalidate();const ticket=epoch, code=input.value;
    if(code.length>600000 || !code.trim()) {setCopyText(status, () => t('거래 코드의 형식·버전·길이를 확인하세요. 폰에서 서명한 거래 코드를 다시 복사하세요.'));return;}
    check.disabled=true;setCopyText(status, () => t('거래 내용을 확인하는 중입니다. 아직 보내지 않았습니다.'));
    try {
      ...
      for(const output of value.outputs) {
        const heading=document.createElement('h4');setCopyText(heading, () => t('출력')+' '+(output.index+1));preview.append(heading);
      ...
      setCopyText(status, () => t('아직 보내지 않았습니다. 폰의 내용과 대조한 뒤 확인하고 보내세요.'));
      send.hidden=false;send.disabled=false;
    } catch(error) {if(ticket===epoch)setCopyText(status, () => t(error instanceof Error?error.message:String(error)));}
