...
mkdirSync(root + 'artifacts/desktop-integration', { recursive: true });
let result;
try {
  writeFileSync(path, before.replace(guard, 'if false /* intentional test mutant: removed previous transaction hash binding */'));
  result = spawnSync('cargo', ['test', '--lib', 'companion::tests::previous_transaction_is_verified_against_actual_bytes', '--', '--exact', '--test-threads=1'], { cwd: root + 'src-tauri', encoding: 'utf8', timeout: 120000 });
} finally { writeFileSync(path, before); }
const output = (result?.stdout ?? '') + (result?.stderr ?? '');
writeFileSync(root + 'artifacts/desktop-integration/native-mutant-red.txt', output);
