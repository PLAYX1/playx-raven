...
(() => {
  ...
  const css = `
    ...
    /* 🔴 화면 위 오른쪽에 붙는 것들도 같이 비켜 준다. 실측(2026-09-06 대표 사진):
       지갑의 「잠그기」가 「?」에 반쯤 덮여 있었다 — 누를 수 없는 상태였다.
       .topbar 는 화면 폭을 다 쓰므로 오른쪽 끝이 그대로 깔린다.
       (주의: 이 문자열은 템플릿 리터럴이라 백틱을 쓰면 안 된다 — 실제로 깨뜨렸다.) */
