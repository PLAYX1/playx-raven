...
The literal template attribute is `Key="Software\\{{manufacturer}}\PLAY X Raven"`: two backslashes before the Handlebars expression, one before the fixed product name. Handlebars consumes the escape before an expression but preserves repeated separators before literal text. Keeping two before `PLAY X Raven` produced six invalid registry paths and Windows linker ICE03/LGHT0204 failures. The previous synthetic test normalized those malformed paths and missed this defect; registry render checks must never normalize them.

The regression invokes Rust Handlebars **6.3.0**, pinned by the official [CLI 2.11.4 Cargo.lock](https://github.com/tauri-apps/tauri/blob/tauri-cli-v2.11.4/Cargo.lock), using the same [MSI bundler rendering sequence](https://github.com/tauri-apps/tauri/blob/tauri-cli-v2.11.4/crates/tauri-bundler/src/bundle/windows/msi/mod.rs): register the template with `no_escape`, render once, then render that output through a fresh default Handlebars instance. All six registry attributes must equal the exact legacy path above, including single separators. The uncorrected template failed this assertion with exit code 1 before the fix.
...
node scripts/desktop-installer-identity.test.mjs --mutant-wix-registry
```

The last three must fail; mutations exist only in memory. The default check requires Rust/Cargo and resolves public renderer dependencies in a temporary fixture, which is removed with its compiled files after the check. It executes only the renderer, never the app or an installer. Semantic tests execute a strict subset of the actual NSIS compatibility instructions against synthetic registry/shortcut maps. The optional compile check uses local `makensis` to compile the modified code with fixture helper macros, then removes the resulting PE **without executing it**. These checks do not substitute for MSI linking or installing/updating on Windows. Hash reconstruction checks that every other byte still matches upstream; review and update these templates deliberately when the CLI version changes.
