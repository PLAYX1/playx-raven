...
pub(crate) const FEE: f64 = 0.1;

/// 개발비를 **출력으로 만들 수 있는 최소액**. 이보다 작으면 안 넣는다 —
/// 넣으면 그 출력을 보내는 값이 더 나오고, 노드가 거절할 수도 있다.
///
/// 🔴 **최소 「징수액」이 아니라 최소 「출력액」이다.** 1% 기준으로 상품값이
///    1 RVN 미만이면 개발비가 **0** 이 된다. 값을 낮출 때의 함정이라
///    `auction.rs` 도 이 값을 보고 시작가 하한을 정한다.
pub(crate) const MIN_DEV_FEE: f64 = 0.01;
...
pub async fn swap_take(
    ...
    if fee_dev >= MIN_DEV_FEE && !fee_addr.is_empty() {
