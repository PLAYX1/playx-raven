...
//! 둔다.** 쌓인 것은 두 길로 나간다 — 사장이 「보내기」를 누르거나,
//! `start_auto_pay` 가 3분마다 보다가 **지갑이 이미 풀려 있을 때** 정해진
//! 개발비 주소로 자동으로 보낸다(0.01 RVN 이상일 때만).
//!
//! 자동 송금을 위해 지갑을 일부러 열어 두지는 않는다. 암호를 파일에 적지도
//! 않는다. 가게 컴퓨터의 지갑 암호를 24시간 풀어 두는 것보다 1% 를 며칠
//! 늦게 받는 쪽이 낫다. 받는 주소는 소스에 박혀 있고, 이 장부 파일은
//! 금액만 적는다 — 파일을 고쳐 받는 곳을 바꿀 수는 없다.
//!
//! ## 🔴 적힌 것은 지워지지 않는다
//!
