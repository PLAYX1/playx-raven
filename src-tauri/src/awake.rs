...
//! 「지갑」(0.4.8)도 「돕기」와 같다 — 쓸 때 열고 닫는 사람의 노트북이다.
//!
//! ## 🔴 예외 하나 — 다시 훑는 동안
//!
...
mod tests {
    ...
    /// 🔴 지갑만 쓰는 사람(0.4.8)은 쓸 때 열고 닫는다. 그 노트북을 밤새 깨워
    ///    두면 받고 보내려고 깐 프로그램이 배터리를 태우는 프로그램이 된다.
    ///    다시 훑는 동안만은 다른 모드와 같이 붙잡는다 — 잠들면 처음부터다.
    #[test]
    fn 지갑도_안_건드린다() {
        assert!(!should_hold("wallet", false), "지갑만 쓰는 사람의 배터리도 그 사람 것이다");
        assert!(should_hold("wallet", true), "다시 훑는 동안은 지갑이어도 붙잡는다");
    }
}
