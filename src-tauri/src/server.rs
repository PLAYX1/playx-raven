...
/// 이 컴퓨터가 들고 있는 자산 파일 목록. **다른 노드가 도우려고 물어본다.**
///
/// 🔴 숨길 것이 없다 — 자산 이름도 IPFS 주소도 이미 체인에 공개돼 있다.
///    없던 것은 「한 번에 받는 길」뿐이었고, 그게 없어서 서로 못 도왔다.
async fn api_pins() -> impl IntoResponse {
    Json(crate::peers::my_cids().await)
}
...
async fn outside_gate(
    ...
    if outside_blocked(&state, req.headers(), req.uri().path()) {
        // 폰 앱·직원 웹은 `/api/*` 의 401 REMOTE_OFF JSON 으로 「원격 관리 꺼짐」을 안내한다.
        // 화면 길(HTML)은 사람이 읽는 글로 막는다.
        if req.uri().path().starts_with("/api/") {
            return (StatusCode::UNAUTHORIZED, Json(auth_fail_json(AuthFail::RemoteOff))).into_response();
        }
...
pub async fn start_phone_server(
    ...
    // 🔴 **켤 때 한 번 쓴다.** 되돌리기가 풀어 놓은 것을 안 치우던 시절에
    //    남은 것들이 있다 — 실측으로 일주일 넘게 **잠금 풀린 지갑과 간판
    //    열쇠**가 놓여 있었다. 이미 판올림한 사람의 컴퓨터에서도 지워야
    //    한다. 고친 뒤부터만 안 쌓이게 하는 것으로는 모자란다.
    crate::recover::청소();
