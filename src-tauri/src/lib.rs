...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            member_privacy::member_groups_get,
            member_privacy::member_groups_set,
            pass::member_groups_assign,
            relay::relay_status,
            devfee::fee_pay,
            ...
            autostart::autostart_audit,
            autostart::autostart_tidy,
        ...
        .run(|_app, event| {
            ...
            if matches!(event, tauri::RunEvent::Exit) {
                ...
                // 🔴 **바깥 연결도 끊는다.** 여태 `tunnel_stop()` 은 종료 경로
                //    어디에서도 안 불렸다. 그래서 사장이 앱을 껐는데도
                //    cloudflared 가 남아 **가게 화면이 인터넷에 계속 열려**
                //    있었다. 다음에 앱을 켤 때에야 같은 포트를 문 것을 골라
                //    죽였다 — 그때까지는 문이 열린 채다.
                //
