...
//!
//! ## 🔴 왜 이 파일은 표준 경로로 안 옮기는가
//!
//! 신원의 경로표는 `identity.rs` 에 있고, 거기 가게 간판 자리
//! (`m/44'/175'/7'/1'/0'`)도 정해 뒀다. 그런데 **여기는 안 옮긴다.**
//!
//! 간판 공개키는 **체인의 가게 정보에 박혀 있다.** 열쇠만 바꾸면 손님이
//! 가게를 못 찾는다. 같이 바꾸려면 자산 재발행이고 **100 RVN 이 탄다.**
//! 그리고 지금 열쇠도 어차피 12단어에서 나온다 — 표준 경로가 아닐 뿐,
//! **복구는 된다.** 엘레강스 하나 때문에 사장에게 100 RVN 을 또 태우라고
//! 할 이유가 없다.
//!
//! 옮기고 싶으면 `shopmove.rs` 가 값을 먼저 보여 주고 사장이 고른다.
//! 그때도 **체인이 먼저, 파일이 나중**이다.
//!
//! ⚠️ 그래서 이 파일의 표식 해시 방식은 **레거시가 아니라 현역**이다.
//!    향후 네이티브 APP 이 가게 간판을 다루려면 이 방식도 구현해야 한다.
//!    `identity_paths()` 가 화면에 그 사실을 그대로 적어 준다.
//!
//! ## 🔴 이 파일은 비밀이다
//!
...
///
/// 🔴 **이제 새 이름은 여기서 안 나온다.** 사람 열쇠는 표준 경로
/// (`identity::PATH_PERSON`)로 옮겼다 — 그래야 폰·웹 지갑과 같은 사람이 된다.
/// 이 표식은 **그 전에 만들어진 이름을 되살리기 위해서만** 남아 있다.
/// 지우면 그 이름으로 쓴 글이 전부 고아가 된다. 그래서 안 지운다.
pub const SEED_TAG_TALK: &str = "PLAYX-RAVEN-TALKKEY-v1";

/// 컴파일 시점 문자열 비교. `identity.rs` 의 `same` 과 같은 이유로 있다.
...
// 🔴 표식을 고치면 **여기서 빌드가 멈춘다.**
//    `SEED_TAG` 를 고치면 체인에 박힌 공개키와 어긋나 가게가 죽고,
//    `SEED_TAG_TALK` 를 고치면 옛 이름을 되살릴 길이 사라진다.
...
const _: () = assert!(
    ...
    "옛 이름 표식을 바꾸면 그 이름으로 쓴 글을 영영 못 되찾는다."
);
