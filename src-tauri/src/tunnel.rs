...
/// 앱을 끌 때 바깥 연결을 끊는다.
///
/// 🔴 여태 `tunnel_stop()` 은 **종료 경로 어디에서도 안 불렸다.** 사장이 앱을
///    껐는데 cloudflared 가 남아 **가게 화면이 인터넷에 계속 열려** 있었다.
///    다음에 앱을 켤 때에야 같은 포트를 문 것을 골라 죽였다 — 그때까지는
///    문이 열린 채다. 껐다고 믿는 사람에게 열린 문은 가장 나쁜 구멍이다.
///
/// ⚠️ 실패해도 앱은 계속 꺼진다. 종료를 막으면 사장이 강제로 끄게 되고,
///    그러면 장부가 상한다.
pub fn stop_on_exit() {
    let _ = tunnel_stop();
}
