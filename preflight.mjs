...
// ⑭ **웹 지갑이 물건을 살 때 개발비가 진짜로 붙는가.**
//
// 🔴 2026-09-06 실측: 화면에는 「개발비 1%」가 뜨는데 **서명에는 안 들어갔다.**
//    `openBuyFromHash` 가 `buying` 을 세운 **뒤** `openSend()` 를 불렀는데,
//    그 함수 안에 `buying = null` 이 있다(한 번 산 뒤 그냥 보내기에도 붙는
//    것을 막는 줄). 그래서 세우자마자 지워졌고, **웹에서 물건을 사도
//    개발비가 한 푼도 안 걷혔다.** 오류는 나지 않는다 — 그냥 안 걷힌다.
//
//    한 줄 순서 문제라 다시 뒤집히기 쉽다. 그래서 검사로 못 박는다.
{
  const src = read("web/wallet.src.ts");
  const m = /function openBuyFromHash\(\)[\s\S]*?\n\}/.exec(src);
  if (!m) fail("웹 지갑에서 `openBuyFromHash` 를 못 찾았다 — 이 검사가 헛돈다", []);
  else {
    // 🔴 **주석을 먼저 걷어낸다.** 안 걷으면 이 결함을 설명한 주석 안의
    //    `openSend()` 글자를 먼저 찾아서, 순서가 뒤집혀도 통과한다.
    //    이 저장소가 같은 함정을 또 밟았다(2026-09-06). 자기 소스를 읽는
    //    검사는 **반드시 주석을 지우고** 봐야 한다.
    const body = m[0].replace(/\/\/[^\n]*/g, "").replace(/\/\*[\s\S]*?\*\//g, "");
    const 연다 = body.indexOf("openSend()");
    const 세운다 = body.indexOf("buying = {");
    if (연다 < 0 || 세운다 < 0)
      fail("사는 중 표시나 보내기 화면 여는 줄이 사라졌다", [
        `openSend() ${연다 < 0 ? "없음" : "있음"} · buying = { ${세운다 < 0 ? "없음" : "있음"}`,
      ]);
    else if (세운다 < 연다)
      fail("웹에서 물건을 사도 개발비가 안 걷힌다", [
        "`buying = {…}` 이 `openSend()` **앞**에 있다 — openSend 가 곧바로 null 로 지운다",
        "화면에는 1% 가 뜨는데 서명에는 안 들어간다. 순서를 바꿔라",
      ]);
    else ok("웹 지갑이 물건 살 때 개발비를 붙임 (사는 중 표시가 화면을 연 뒤에 선다)");
  }
}
